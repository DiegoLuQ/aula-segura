...
- Two reminders for the same student never land on the same day. Two mechanisms enforce this, both in `notifications.py`: (a) notifying a phase calls `cancelar_fases_anteriores()`, which cancels the still-`pendiente` sends of every phase before it in `ORDEN_FASES` (the process moved on) — this runs from `crear_notificacion` and from `registrar_consejo`; and (b) `_resolver_choques()` pushes any remaining date that is already taken (`fechas_ocupadas()`) to the next free business day, keeping the plan's order. `GET /estudiantes/{id}/fechas-ocupadas?etapa=` exposes the same occupied dates so the modal preview in `registro.html` shows the exact dates the backend will schedule.
- Editing a phase date on an already-notified student re-plans it: `update_estudiante` diffs the `ETAPA_FECHA_ATTR` fields and calls `reprogramar_envios_por_cambio_fecha()` for each one that changed. Only `pendiente` sends move (already-sent ones stay as history and keep occupying their day); clearing the date cancels the pending sends. Without this the `EnvioProgramado` rows kept firing on the old dates.
